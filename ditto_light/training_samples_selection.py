...
import time
import pickle
from ditto_light.battleships import battleships_graph
from ditto_light.DTAL import DTAL
import argparse
import json
import torch
import os
...
class TopKSelection:
    ...
    def get_available_pool_ids(self):
        ...
        if self.iter == 0:
            ...
            # open the pool ids file and return the available ids
    ...
    def get_available_pool(self):
        ...
        # filter the available pool ids and place the corresponding
        # pairs in a pools dictionary
    ...
    def handle_from_iter(self):
        ...
        for curr_iter in range(1, self.iter):
            for file in files_list:
                # TODO: write to this file before calling with the previous selected (in-domain)
    ...
    def get_new_train_ids(self):
        ...
        # save to file the current selected samples
    ...
    def read_source_dataset(self):
        ...
        BEACON Patch:
        -------------
        In the original Battleship system, this function loads the "source"
        dataset (e.g., Walmart-Amazon) using configs.json. BEACON does NOT use 
        Magellan dataset configs, and our wrapper manually sets seed/pool, so 
        we *never* want to load any external source data.

        Therefore:
            - If without_DA=True (default for our BEACON use case),
            we skip ALL source dataset logic and return an empty list.
            - This completely prevents find_file() / configs.json lookups.
        """

        # BEACON mode: do NOT load any source dataset; avoid configs.json entirely.
        if self.without_da:
            return []

        # The code below should never run for BEACON, but we leave it intact
        # for compatibility with the original Battleship system.
        current_train = []
    ...
    ##############################################################
    # NEW METHOD: BEACON-style AL with true domain-aware seed/pool
    ##############################################################
    @classmethod
    def active_select_from_domain_dict(
        cls,
        data_dict,
        domain,
        budget,
        tmp_dir="tmp_battleship/",
        mode="battleships_ws_b_alpha=1.0_beta=1.0",
        seed=0,
    ):
        """
        BEACON-Battleship wrapper:
        - Seed = in-domain train
        - Pool = out-of-domain (train+val+test)
        - Forces seed/pool split at iter=0
        - Runs model training/inference at iter=0 (required by Battleship)
        - Runs one Battleship AL iteration at iter=1
        - Returns only the selected OOD samples
        """
        # fake task layout
        os.makedirs(tmp_dir, exist_ok=True)
        fake_task = f"{domain}/0"
        fake_source_task = fake_task
        fake_files_path = tmp_dir + "/"
        fake_output_path = tmp_dir + "/output/"
        os.makedirs(fake_output_path, exist_ok=True)

        # form pool of first all in domain, then out-domain data
        in_train = data_dict[domain][0]
        ood_pool = []
        for dom in data_dict:
            if dom != domain:
                # include all train+val+test OOD data
                ood_pool.extend(data_dict[dom][0])
                #ood_pool.extend(data_dict[dom][1])
                #ood_pool.extend(data_dict[dom][2])
        combined = in_train + ood_pool
        
        # write current selected indices
        initial_selected_indices = {x for x in range(len(in_train))}
        ind_file = open(tmp_dir + "/output/" + mode + '/pkl_files/' + "selected_k_pool_to_original_iter" + str(1) + '_seed' + str(seed) + '.pkl', 'wb')
        pickle.dump(initial_selected_indices, ind_file)
        ind_file.close()

        # write current train
        init_train_pkl_file_h = open(fake_files_path + 'current_train.pkl', 'wb')
        pickle.dump(initial_selected_indices, init_train_pkl_file_h)
        init_train_pkl_file_h.close()
        with open(fake_files_path + 'current_train.txt', 'wb') as init_train_txt_file_h:
            init_train_pkl_file_h.writelines(in_train)

        # 

        # --------------------------
        # 3. Instantiate Battleship at iteration=0
        # --------------------------
        selector = cls(
            task=fake_task,
            SourceTask=fake_source_task,
            k=budget,                 # how many OOD samples to select
            iteration=0,              # start at iteration 0
            mode=mode,
            files_path=fake_files_path,
            orig_train_path=orig_train_path,
            seed=seed,
            iterations=2,             # we will run iter=0 then iter=1
            output_path=fake_output_path,
            from_iter=0,
        )

        # Ensure no Magellan source loading
        selector.without_da = True

        # --------------------------
        # 4. Force seeds = in-domain, pool = OOD
        # --------------------------
        num_in = len(in_train)
        num_total = len(combined)

        selector.current_train_ids = set(range(num_in))                 # seed = in-domain
        selector.available_pool_ids = set(range(num_in, num_total))     # OOD = pool

        # rebuild pool structures based on OOD only
        selector.available_pool, selector.pool_to_original, selector.pool_labels_dict = \
            selector.get_available_pool()

        # write corrected seed/pool PKLs
        selector.update_train_pkl(selector.current_train_ids)
        selector.update_pool_pkl(set())     # pool already set

        # --------------------------
        # 5. REQUIRED: Run model train+inference for iter=0
        # --------------------------
        selector.iter = 1
        _ = selector.find_top_k()    # trains model & writes pooler outputs

        # --------------------------
        # 6. NOW run true AL selection at iter=1
        # --------------------------
        selector.iter = 2
        selected_ids, _, _ = selector.get_new_train_ids()   # calls find_top_k()

        # Only keep OOD-selected samples (exclude in-domain)
        selected_k_only = selected_ids.difference(set(range(num_in)))

        # Map back to raw lines
        selected_pairs = [combined[i] for i in selected_k_only]
