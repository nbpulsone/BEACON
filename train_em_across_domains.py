...
#TODO TEST MINIMIZE HUGGING FACE REQUESTS OPTIMIZATION
...
if __name__=="__main__":
    ...
    parser.add_argument("--keep_lower_budgets", dest="keep_lower_budgets", action="store_true") #TODO use this and test new saving code in ditto.py
    ...
    method = hp.method
    budget = hp.budget
    domain_data, tasks = process_budget(general_task, method, budget, tokenizer, lm=hp.lm)
    
    # run training and evaluation for all tasks
    for task in tasks:
        hp.task = task
        print(f"\nTRAINING: {hp.task}\n")
        ...
        train(train_dataset,
            valid_dataset,
            test_dataset,
            run_tag, hp, domain_data, shared_model=shared_model)
